...
impl Backdrop {
    ...
    /// Every backdrop, for the settings dialog to offer.
    pub const ALL: [Self; 4] = [Self::Scene, Self::Checker, Self::Black, Self::White];

    /// What it is called in the settings file, and read back from it.
    ///
    /// The default has a name here even though `name` gives it none: the
    /// status line says nothing about the backdrop nobody chose, but the file
    /// has to be able to write down that it was chosen deliberately.
    ...
    /// The backdrop that keyword names, if it names one.
    pub fn from_keyword(text: &str) -> Option<Self> {
        Self::ALL.into_iter().find(|backdrop| backdrop.keyword() == text)
    }

    /// What the settings dialog calls it, where the default needs a word.
    pub fn label(self) -> &'static str {
        match self {
            Self::Scene => "the viewer's own",
            other => other.name().unwrap_or_default(),
        }
    }
